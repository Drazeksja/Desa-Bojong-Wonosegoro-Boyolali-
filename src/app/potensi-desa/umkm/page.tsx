...
const UMKM_LIST: Umkm[] = [
  {
    ...
    highlights: ["± 30 bungkus diproduksi per hari", "Bahan baku datang 4× seminggu dari supplier", "Keripik pisang jadi produk terlaris"],
    marketArea: "Mitra pasar lokal & pelanggan tetap dari Sukoharjo (± 100 bungkus/pesanan)",
  ...
  },
  {
    ...
    highlights: ["Bahan baku utama kayu jati pilihan", "Menerima pesanan custom penuh", "Pernah ekspor & jadi pemasok toko mebel"],
  ...
  },
  {
    ...
    highlights: ["Produksi dimulai pukul 02.00 WIB", "Penjual keliling bersistem komisi", "Belum memiliki merek/label dagang"],
  ...
  },
  {
    ...
    highlights: ["± 150 kg singkong diolah per hari", "Produksi 4.000–5.000 biji per hari", "Dikerjakan oleh ± 7 pekerja"],
    marketArea: "Distribusi lewat pedagang keliling sejak pukul 02.00–04.00 WIB",
  ...
  },
  {
    ...
    highlights: ["Tanpa pengawet, tahan hingga 6 bulan", "Sudah bersertifikat kesehatan", "Pernah tampil di pameran UMKM Solo"],
...
function UmkmPhoto({ slug, name }: { slug: string; name: string }) {
  const [failed, setFailed] = useState(false);
  const src = `/images/umkm/${slug}.webp`;
  ...
  return (
    <div className="umkm-photo-frame">
      ...
      <span className="umkm-photo-category">{/* filled via parent */}</span>
    </div>
  );
}
  ...
  return (
    <div className="umkm-card h-100">
      <div className="umkm-photo-wrap">
        <UmkmPhoto slug={item.slug} name={item.name} />
      ...
      <div className="umkm-card-body">
        ...
        <p className="umkm-owner">
          {item.owner}
          {item.established && <span className="umkm-established"> · {item.established}</span>}
        </p>
        ...
        {item.whatsapp && (
          <a
            href={`https://wa.me/62${item.whatsapp.replace(/^0/, "").replace(/-/g, "")}`}
            target="_blank"
    ...
    <main className="pb-5 bg-white min-vh-100 pt-5">
      <div className="container mt-5 pt-4" style={{ maxWidth: "1180px" }}>
        ...
        <div className="mb-5 text-center border-bottom border-secondary border-opacity-25 pb-4">
          ...
          <p className="text-muted mx-auto mt-3" style={{ maxWidth: "640px" }}>
            Mengenal lebih dekat pelaku usaha mikro, kecil, dan menengah yang menjadi
            penggerak ekonomi warga Desa Bojong dari kudapan tradisional hingga
            kerajinan kayu.
          </p>
        </div>
